CREATE PROCEDURE [dbo].[PR_PLANILHA_ESTOQUE_INICIAL]
	@DISPLAYLENGTH INT,
	@DISPLAYSTART INT,
	@SORTCOL INT,
	@SORTDIR NVARCHAR(10),
...
BEGIN
	...
	With CTE_PlanilhaEstoqueInicial as
	(
		Select 
			ROW_NUMBER() over 
			(
		...
		FROM
			...
			INNER JOIN Estoque ON Estoque.IdProduto = Prod.Id AND Estoque.Tipo = 'I'
		WHERE 
		(
			Prod.Ativo = 1 AND
	...
	Select * from CTE_PlanilhaEstoqueInicial where @ListarTodos = 1 OR (@ListarTodos  = 0 AND RowNum > @FirstRec and RowNum <= @LastRec)
END