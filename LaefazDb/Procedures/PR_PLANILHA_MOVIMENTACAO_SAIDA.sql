CREATE PROCEDURE [dbo].[PR_PLANILHA_MOVIMENTACAO_SAIDA]
	@DISPLAYLENGTH INT,
	@DISPLAYSTART INT,
	@SORTCOL INT,
	@SORTDIR NVARCHAR(10),
...
BEGIN
	...
	With CTE_PlanilhaMovimentacaoSaida as
	(
		Select 
			ROW_NUMBER() over 
			(
		...
		FROM
			...
			INNER JOIN Movimentacao ON Movimentacao.IdProduto = Prod.Id AND Movimentacao.Tipo = 'S'
		WHERE 
		(
			Prod.Ativo = 1 AND
	...
	Select 
		...
		ValorTotal,
		CFOP	
	from CTE_PlanilhaMovimentacaoSaida where @ListarTodos = 1 OR (@ListarTodos  = 0 AND RowNum > @FirstRec and RowNum <= @LastRec)
END