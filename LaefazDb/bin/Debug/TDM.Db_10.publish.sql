		...
		SELECT 
			...
			(
				order by
					...
					case when (@SortCol = 5 and @SortDir='asc') then (SELECT CAST(COUNT(TestData.IdStatus) AS INT) FROM TestData WHERE TestData.IdDataPool = DataPool.Id AND TestData.IdStatus=2) end asc,
					case when (@SortCol = 5 and @SortDir='desc') then (SELECT CAST(COUNT(TestData.IdStatus) AS INT) FROM TestData WHERE TestData.IdDataPool = DataPool.Id AND TestData.IdStatus=2) end desc,
					...
					case when (@SortCol = 7 and @SortDir='asc') then (SELECT CAST(COUNT(TestData.IdStatus) AS INT) FROM TestData WHERE TestData.IdDataPool = DataPool.Id AND TestData.IdStatus=4) end asc,
					case when (@SortCol = 7 and @SortDir='desc') then (SELECT CAST(COUNT(TestData.IdStatus) AS INT) FROM TestData WHERE TestData.IdDataPool = DataPool.Id AND TestData.IdStatus=4) end desc
			...
			(SELECT CAST(COUNT(TestData.IdStatus) AS INT)FROM TestData WHERE TestData.IdDataPool = DataPool.Id AND TestData.IdStatus=2) QtdDisponivel,
			(SELECT CAST(COUNT(TestData.IdStatus) AS INT)FROM TestData WHERE TestData.IdDataPool = DataPool.Id AND TestData.IdStatus=3) QtdReservada,
			(SELECT CAST(COUNT(TestData.IdStatus) AS INT)FROM TestData WHERE TestData.IdDataPool = DataPool.Id AND TestData.IdStatus=4) QtdUtilizada,
			0 Farol,
...
INSERT INTO Script (IdAUT, Descricao)
(SELECT Id, 'ABRIR OSS DE ATIVAÇÃO DE FIXO' FROM AUT WHERE Descricao = 'STC');

INSERT INTO Script (IdAUT, Descricao)
(SELECT Id, 'AJA - CADASTRA - ALTERAR CLIENTE' FROM AUT WHERE Descricao = 'SAC');

INSERT INTO Script (IdAUT, Descricao)
(SELECT Id, 'AJA - CADASTRA CLIENTE - CPF' FROM AUT WHERE Descricao = 'SAC');
...
BEGIN
INSERT INTO DEMANDA(Descricao, Complemento)VALUES('PRJ00028303_ENT00000000', 'TeD - Antecipação - OS não concluida');
END
	/*
Post-Deployment Script Template							
--------------------------------------------------------------------------------------
...
INSERT INTO Script_CondicaoScript (IdScript, IdCondicaoScript)  
(SELECT scr.Id, con.Id FROM Script scr, CondicaoScript con WHERE scr.Descricao = 'ABRIR OSS DE ATIVAÇÃO DE FIXO' AND con.Descricao = 'COM CAMPANHA');
INSERT INTO Script_CondicaoScript (IdScript, IdCondicaoScript)  
(SELECT scr.Id, NULL FROM Script scr WHERE scr.Descricao = 'CRIAÇÃO DE CONTATO E CONTA PF');
INSERT INTO Script_CondicaoScript (IdScript, IdCondicaoScript)  
(SELECT scr.Id, NULL FROM Script scr WHERE scr.Descricao = 'AJA - CADASTRA CLIENTE - CPF');
...
(SELECT scr.Id, NULL,'/Execution/ExecutionLists/REALIZAR_INCLUSAO_DE_ENDERECO_FIBRA','C:\Tosca_Projects\Tosca_Workspaces\TDM\remote_exec\realizar-inclusao-de-endereco-fibra.tcs','C:/Tosca_Projects/Tosca_Workspaces/TDM/remote_exec/Reports/report.txt',
...
INSERT INTO Script_CondicaoScript (IdScript, IdCondicaoScript, ListaExecucaoTosca, CaminhoArquivoTCS, DiretorioRelatorio, QueryTosca)  
(SELECT scr.Id, NULL,'/Execution/ExecutionLists/REALIZAR_CRIACAO_DE_CONTATO','C:\Tosca_Projects\Tosca_Workspaces\TDM\remote_exec\realizar-criacao-de-contato.tcs','C:/Tosca_Projects/Tosca_Workspaces/TDM/remote_exec/Reports/report.txt',
...
INSERT INTO Script_CondicaoScript (IdScript, IdCondicaoScript, ListaExecucaoTosca, CaminhoArquivoTCS, DiretorioRelatorio, QueryTosca)  
(SELECT scr.Id, NULL,'/Execution/ExecutionLists/CRIAR_PERFIL_DE_FATURAMENTO','C:\Tosca_Projects\Tosca_Workspaces\TDM\remote_exec\criar-perfil-de-faturamento.tcs','C:/Tosca_Projects/Tosca_Workspaces/TDM/remote_exec/Reports/report.txt',
...
INSERT INTO Script_CondicaoScript (IdScript, IdCondicaoScript, ListaExecucaoTosca, CaminhoArquivoTCS, DiretorioRelatorio, QueryTosca)  
(SELECT scr.Id, NULL,'/Execution/ExecutionLists/CRIAR_ANALISE_DE_CREDITO','C:\Tosca_Projects\Tosca_Workspaces\TDM\remote_exec\criar-analise-de-credito.tcs','C:/Tosca_Projects/Tosca_Workspaces/TDM/remote_exec/Reports/report.txt',
 'SELECT DISTINCT top 10000 tdt.Id AS ID_TEST_DATA,
                    scr.Descricao AS SCRIPT,
                    dem.Descricao AS DEMANDA,
                    tdt.CasoTesteRelativo AS CASO_TESTE,
...
LEFT JOIN Script scr ON scrcon.IdScript = scr.Id
LEFT JOIN CondicaoScript con ON scrcon.IdCondicaoScript = con.Id
WHERE scr.Descricao = 'ABRIR OSS DE ATIVAÇÃO DE FIXO' AND con.Descricao = 'COM CAMPANHA'), tpar.Id, 1 
FROM Parametro par, TipoParametro tpar
...
LEFT JOIN Script scr ON scrcon.IdScript = scr.Id
LEFT JOIN CondicaoScript con ON scrcon.IdCondicaoScript = con.Id
WHERE scr.Descricao = 'ABRIR OSS DE ATIVAÇÃO DE FIXO'), tpar.Id, 1 
FROM Parametro par, TipoParametro tpar
WHERE par.Descricao = 'EVIDENCIA: RESULTADO ESPERADO'
AND tpar.Descricao = 'ENTRADA');
...
LEFT JOIN Script scr ON scrcon.IdScript = scr.Id
LEFT JOIN CondicaoScript con ON scrcon.IdCondicaoScript = con.Id
WHERE scr.Descricao = 'AJA - CADASTRA CLIENTE - CPF'), tpar.Id, 1 
FROM Parametro par, TipoParametro tpar
WHERE par.Descricao = 'EVIDENCIA: RESULTADO ESPERADO'
AND tpar.Descricao = 'ENTRADA');
...
INSERT INTO TDM (Descricao, TdmPublico) VALUES ('TDM TESTE RELEASE MAIO', 0);
...
(SELECT (SELECT Id FROM TDM WHERE Descricao = 'TDM TESTE RELEASE MAIO'), (SELECT Id FROM AUT WHERE Descricao = 'SAC'), 
(SELECT Id FROM Demanda WHERE Descricao = 'TeD - Antecipação - OS não concluida'), 
...
INSERT INTO Script_CondicaoScript_Ambiente (IdScript_CondicaoScript, IdAmbienteVirtual,IdAmbienteExecucao)
(SELECT (SELECT scrcon.Id FROM Script_CondicaoScript scrcon LEFT JOIN Script scr ON scrcon.IdScript = scr.Id WHERE scr.Descricao = 'CRIAÇÃO DE CONTATO E CONTA PF'),
(SELECT Id FROM AmbienteVirtual WHERE Descricao = 'VDI01'),1);
INSERT INTO Script_CondicaoScript_Ambiente (IdScript_CondicaoScript, IdAmbienteVirtual,IdAmbienteExecucao)
(SELECT (SELECT scrcon.Id FROM Script_CondicaoScript scrcon LEFT JOIN Script scr ON scrcon.IdScript = scr.Id WHERE scr.Descricao = 'CRIAÇÃO DE CONTATO E CONTA PF'),
(SELECT Id FROM AmbienteVirtual WHERE Descricao = 'VDI02'),1);
...
INSERT INTO Script_CondicaoScript_Ambiente (IdScript_CondicaoScript, IdAmbienteVirtual,IdAmbienteExecucao)
(SELECT (SELECT scrcon.Id FROM Script_CondicaoScript scrcon LEFT JOIN Script scr ON scrcon.IdScript = scr.Id WHERE scr.Descricao = 'ABRIR OSS DE ATIVAÇÃO DE FIXO'),
(SELECT Id FROM AmbienteVirtual WHERE Descricao = 'VDI01'),1);
INSERT INTO Script_CondicaoScript_Ambiente (IdScript_CondicaoScript, IdAmbienteVirtual,IdAmbienteExecucao)
(SELECT (SELECT scrcon.Id FROM Script_CondicaoScript scrcon LEFT JOIN Script scr ON scrcon.IdScript = scr.Id WHERE scr.Descricao = 'ABRIR OSS DE ATIVAÇÃO DE FIXO'),
(SELECT Id FROM AmbienteVirtual WHERE Descricao = 'VDI02'),1);
...
INSERT INTO Script_CondicaoScript_Ambiente (IdScript_CondicaoScript, IdAmbienteVirtual,IdAmbienteExecucao)
(SELECT (SELECT scrcon.Id FROM Script_CondicaoScript scrcon LEFT JOIN Script scr ON scrcon.IdScript = scr.Id WHERE scr.Descricao = 'AJA - CADASTRA CLIENTE - CPF'),
(SELECT Id FROM AmbienteVirtual WHERE Descricao = 'VDI01'),1);
INSERT INTO Script_CondicaoScript_Ambiente (IdScript_CondicaoScript, IdAmbienteVirtual,IdAmbienteExecucao)
(SELECT (SELECT scrcon.Id FROM Script_CondicaoScript scrcon LEFT JOIN Script scr ON scrcon.IdScript = scr.Id WHERE scr.Descricao = 'AJA - CADASTRA CLIENTE - CPF'),
(SELECT Id FROM AmbienteVirtual WHERE Descricao = 'VDI02'),1);
...
(SELECT parscr.Id FROM ParametroScript parscr 
	...
	LEFT JOIN Script scr ON scrcon.IdScript = scr.Id
	WHERE par.Descricao = 'CPF' AND scr.Descricao='AJA - CADASTRA - ALTERAR CLIENTE'), '90137414013');	
...
(SELECT parscr.Id FROM ParametroScript parscr 
	...
	LEFT JOIN Script_CondicaoScript scrcon ON parscr.IdScript_CondicaoScript = scrcon.Id
	LEFT JOIN Script scr ON scrcon.IdScript = scr.Id
	WHERE par.Descricao = 'USUARIO SISTEMA' AND scr.Descricao='AJA - CADASTRA - ALTERAR CLIENTE'), 'TR538359');
...
(SELECT parscr.Id FROM ParametroScript parscr 
	...
	LEFT JOIN Script_CondicaoScript scrcon ON parscr.IdScript_CondicaoScript = scrcon.Id
	LEFT JOIN Script scr ON scrcon.IdScript = scr.Id
	WHERE par.Descricao = 'SENHA SISTEMA' AND scr.Descricao='AJA - CADASTRA - ALTERAR CLIENTE'), 'qwe14apt');	
...
(SELECT parscr.Id FROM ParametroScript parscr 
	...
	LEFT JOIN Script_CondicaoScript scrcon ON parscr.IdScript_CondicaoScript = scrcon.Id
	LEFT JOIN Script scr ON scrcon.IdScript = scr.Id
	WHERE par.Descricao = 'APLICACAO' AND scr.Descricao='AJA - CADASTRA - ALTERAR CLIENTE'), '09');		
